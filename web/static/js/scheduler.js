...
function renderCapRejTrade(t) {
  const pct = Math.min(100, Math.round((t.required_capital / Math.max(t.buying_power, 1)) * 100));
  const rows = [
    ["Long / short strike", t.long_strike != null && t.short_strike != null ? `${t.long_strike} / ${t.short_strike}` : "—"],
    ["Expiry",              t.expiry || "—"],
    ["DTE",                 t.dte != null ? t.dte + "d" : "—"],
    ["POP",                 t.pop != null ? Number(t.pop).toFixed(1) + "%" : "—"],
    ["EV",                  t.ev != null ? "$" + Number(t.ev).toFixed(3) + " / sh" : "—"],
    ["Max profit / loss",   (t.max_profit != null && t.max_loss != null) ? `$${Number(t.max_profit).toFixed(2)} / $${Number(t.max_loss).toFixed(2)}` : "—"],
    ["Signal score",        t.signal_score != null ? Number(t.signal_score).toFixed(2) : "—"],
    ["Net delta / theta",   (t.net_delta != null && t.net_theta != null) ? `${Number(t.net_delta).toFixed(3)} / ${Number(t.net_theta).toFixed(4)}` : "—"],
    ["IV edge",             t.iv_edge_vp != null ? Number(t.iv_edge_vp).toFixed(2) + " vp" : "—"],
  ];
  const metaRows = rows.map(([k,v]) =>
    `<div class="crj-meta-row"><span class="crj-key">${k}</span><span class="crj-val">${v}</span></div>`
  ).join("");

  return `<div class="crj-card">
    <div class="crj-card-top">
      <div>
        <span class="crj-ticker">${t.ticker}</span>
        <span class="crj-struct">${t.structure}</span>
      </div>
      <span class="crj-badge">Capital feasibility</span>
    </div>
    <div class="crj-card-body">
      <div class="crj-cap-grid">
        <div class="crj-cap-block">
          <div class="crj-cap-label">Required capital</div>
          <div class="crj-cap-amount crj-bad">${fmtMoney(t.required_capital)}</div>
          <div class="crj-bar-wrap">
            <div class="crj-bar"><div class="crj-bar-fill" style="width:${pct}%"></div></div>
            <span class="crj-bar-pct">${pct}% of BP</span>
          </div>
        </div>
        <div class="crj-cap-block">
          <div class="crj-cap-label">Buying power${t.bp_limit < t.buying_power ? " (limit)" : ""}</div>
          <div class="crj-cap-amount crj-ok">${fmtMoney(t.bp_limit || t.buying_power)}</div>
          <div class="crj-shortfall">Short by ${fmtMoney(t.shortfall)}</div>
        </div>
      </div>
      <div class="crj-meta">${metaRows}</div>
    </div>
  </div>`;
}

function renderCapRejDay(dayObj) {
  const trades = (dayObj.trades || []).map(renderCapRejTrade).join("");
  const count  = dayObj.trades.length;
  return `<div class="crj-day">
    <div class="crj-day-hdr">
      <span class="crj-day-date">${dayObj.date}</span>
      <span class="crj-day-count">${count} trade${count !== 1 ? "s" : ""} filtered</span>
    </div>
    <div class="crj-trades">${trades}</div>
  </div>`;
}

async function loadCapitalRejected() {
  const body  = document.getElementById("cap-rej-body");
  const count = document.getElementById("cap-rej-count");
  try {
    const r = await fetch("/api/paper-trades/capital-rejected");
    const d = await r.json();
    if (!d.ok) throw new Error(d.error);
    const days = d.days || [];
    const total = days.reduce((s, g) => s + (g.trades || []).length, 0);
    count.textContent = total ? `${total} total across ${days.length} day${days.length !== 1 ? "s" : ""}` : "";
    if (!days.length) {
      body.innerHTML = `<p class="muted" style="font-size:.85rem">No capital-rejected trades recorded yet — appears after the next morning scan.</p>`;
      return;
    }
    body.innerHTML = days.map(renderCapRejDay).join("");
  } catch(e) {
    body.innerHTML = `<p class="muted state-error" style="font-size:.85rem">Error: ${e.message}</p>`;
  }
}

document.getElementById("cap-rej-refresh-btn").addEventListener("click", loadCapitalRejected);
loadCapitalRejected();
