...
def load_labeled_dataframe() -> pd.DataFrame:
    ...
    if not df.empty:
        print(f"[POP] {len(df)} labeled rows loaded — {enriched_count} enriched with chain Greeks")
    return df
...
def train(out_path=_MODEL_PATH) -> dict:
    ...
    if train_df.empty or test_df.empty:
        return {"ok": False, "error": "Time-based split produced an empty train or test set."}
